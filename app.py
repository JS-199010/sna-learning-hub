...
if not st.session_state['logged_in']:
    ...
    st.markdown("<h2 style='text-align: center; color: #F8FAFC;'>🩺 SNA 麻醉學習中樞</h2>", unsafe_allow_html=True)
...
# --- PAGE 4: 📊 個人學習成果 ---
    ...
    if not scores:
        ...
        pass_rate = int((len(df[df["score"] >= 60]) / total_exams) * 100)
...
# --- PAGE 5: 🔖 教材重點書籤 ---
    ...
    ui_components.render_header(
        "🔖 教材重點書籤", 
        "在此複習您在互動學習模式下所收藏的經典臨床考題與詳細詳解。"
    )
    
    bookmarks = database.get_bookmarks(st.session_state['username'])
    
    if not bookmarks:
        st.info("💡 目前尚無收藏的重點書籤。在「📚 AI 學習與精準測驗」使用互動模式答題時，即可隨時將關鍵考題收藏至此！")
    else:
        for bm in bookmarks:
            bm_id, filename, content, date = bm
            
            ui_components.draw_card_start(f"🔖 收藏自: {filename} ({date})")
            st.markdown(content)
            
            if st.button("🗑️ 移除此收藏", key=f"del_bm_{bm_id}", type="secondary"):
...
# --- PAGE 6: 📊 全院學員統計 (Teacher only) ---
    ...
    ui_components.render_header(
        "📊 全院學員統計與分析儀表板", 
        "供指導教師即時掌握全體學員的學習狀態、測驗成績及個別主題盲點。"
    )
    ...
    if not all_scores:
        ...
        avg_score_all = int(df_all["score"].mean())
