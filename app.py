...
def calculate_gamification(username):
    # Fetch data
    scores = database.get_scores(username)
    bookmarks = database.get_bookmarks(username)
    
    # Calculate stats
    tests_taken = len([s for s in scores if not s[2].startswith("RPG")])
    rpgs_played = len([s for s in scores if s[2].startswith("RPG")])
    bookmark_count = len(bookmarks)
    
    # Calculate EXP
    total_score_sum = sum(float(s[3]) for s in scores if s[3] is not None)
    total_exp = int(tests_taken * 50 + total_score_sum + bookmark_count * 10 + rpgs_played * 100)
    ...
    if tests_taken >= 2:
        valid_scores = [float(s[3]) for s in scores if not s[2].startswith("RPG") and s[3] is not None]
        if len(valid_scores) >= 2:
            latest = valid_scores[0]
            avg_prev = sum(valid_scores[1:]) / len(valid_scores[1:])
            if latest > avg_prev + 20:
                badges.append("🚀 突飛猛進")
                
    level = (total_exp // 500) + 1
    exp_in_level = total_exp % 500
...
if not st.session_state['logged_in']:
    col_spacer1, col_login, col_spacer3 = st.columns([1, 2, 1])
    with col_login:
        col1, col2, col3 = st.columns([1, 1, 1])
        with col2:
            try:
                st.image("assets/medical_logo.png", use_container_width=True)
            except:
                pass # Fallback if image not found
        st.markdown("<h1 style='text-align: center; color: #F8FAFC; line-height: 1.2; font-weight: 800; font-size: 2.2rem; margin-top: 10px;'>輔大醫院SNA學習平台</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; color: #94A3B8; font-size: 1.1rem; margin-bottom: 24px; letter-spacing: 0.05em;'>智能 AI 麻醉教學系統 <span style='color: #6366F1; font-weight: 600;'>by CHS</span></p>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.session_state['role'] == 'student':
        stats = calculate_gamification(st.session_state['username'])
        
        # Check for new badges / gamification updates to trigger toasts
        if 'last_gamification_stats' in st.session_state:
            old_badges = set(st.session_state['last_gamification_stats']['badges'])
            new_badges = set(stats['badges'])
            earned_badges = new_badges - old_badges
            for b in earned_badges:
                st.toast(f"解鎖新成就：{b}！", icon="🎊")
                st.balloons()
            
            old_exp = st.session_state['last_gamification_stats']['exp']
            if stats['exp'] > old_exp:
                st.toast(f"獲得經驗值 +{stats['exp'] - old_exp} XP！", icon="🌟")
                
        st.session_state['last_gamification_stats'] = stats
        
        st.markdown(f"**🌟 等級 {stats['level']}** ({stats['exp_in_level']}/500 XP)")
        st.progress(stats['exp_in_level'] / 500.0)
        if stats['badges']:
                        ...
                        if not text.strip():
                            ...
                            with st.spinner(f"⏳ 步驟 3/3: 正在為 {len(chunks)} 個碎片生成向量並記憶中..."):
                                ...
                                try:
                                    ...
                                    database.supabase.storage.from_("materials").upload(
                                        file=file_bytes, 
        ...
        if not files:
            ...
            for f in files:
                ...
                if st.button(f"🗑️ 刪除 {filename}", key=f"del_{filename}", type="secondary"):
                    ...
                    try:
                        database.supabase.storage.from_("materials").remove([filename])
                    except Exception:
                        pass
...
# --- PAGE: 🎮 臨床模擬 RPG ---
elif choice == "🎮 臨床模擬 RPG":
    ui_components.render_header(
        "🎮 臨床模擬 RPG", 
        "文字角色扮演遊戲：由 AI 擔任主治醫師，給予突發臨床狀況，考驗您的實戰反應！"
    )
    ...
    if not files_list:
        st.warning("目前沒有任何教材可供模擬，請先到「知識教材庫管理」上傳檔案。")
        st.stop()
        
    filenames = [f[0] for f in files_list]
    selected_file = st.selectbox("選擇要模擬的臨床考科/教材", filenames, key="rpg_file")
    
    if 'rpg_state' not in st.session_state:
        st.session_state['rpg_state'] = 'setup'
    if 'rpg_history' not in st.session_state:
        st.session_state['rpg_history'] = []
    
    if st.session_state['rpg_state'] == 'setup':
        if st.button("🎲 開始情境模擬", use_container_width=True):
            with st.spinner("DM 正在從教材中構築臨床情境..."):
                try:
                    scenario = rag_engine.generate_rpg_scenario(selected_file, api_key)
                    st.session_state['rpg_history'] = [{"role": "assistant", "content": scenario}]
                    st.session_state['rpg_state'] = 'playing'
                    st.session_state['rpg_turn'] = 1
                    st.session_state['rpg_file_selected'] = selected_file
                    st.rerun()
                except Exception as e:
                    st.error(f"生成情境失敗：{e}")
                    
    elif st.session_state['rpg_state'] in ['playing', 'finished']:
        if st.session_state.get('rpg_file_selected') != selected_file:
            st.session_state['rpg_state'] = 'setup'
            st.rerun()
            
        for msg in st.session_state['rpg_history']:
            with st.chat_message(msg["role"]):
                st.markdown(msg["content"])
                
        if st.session_state['rpg_state'] == 'playing':
            user_action = st.chat_input("請輸入您的處置方式...")
            if user_action:
                st.session_state['rpg_history'].append({"role": "user", "content": user_action})
                with st.chat_message("user"):
                    st.markdown(user_action)
                    
                with st.chat_message("assistant"):
                    with st.spinner("DM 正在判定您的處置..."):
                        try:
                            response, is_finished = rag_engine.evaluate_rpg_action(
                                selected_file, 
                                st.session_state['rpg_history'], 
                                api_key
                            )
                            st.markdown(response)
                            st.session_state['rpg_history'].append({"role": "assistant", "content": response})
                            st.session_state['rpg_turn'] += 1
                            
                            if is_finished or st.session_state['rpg_turn'] >= 5:
                                st.session_state['rpg_state'] = 'finished'
                                database.save_score(
                                    st.session_state['username'], 
                                    f"RPG模擬 - {selected_file[:15]}", 
                                    100
                                )
                                st.success("🎉 模擬結束！您已獲得 RPG 實戰經驗值！")
                                st.balloons()
                                
                        except Exception as e:
                            st.error(f"判定失敗：{e}")
                            
        if st.session_state['rpg_state'] == 'finished':
            if st.button("🔄 挑戰下一個情境"):
                st.session_state['rpg_state'] = 'setup'
                st.rerun()

# --- PAGE: 📚 AI 學習與精準測驗 (RAG 驅動) ---
elif choice == "📚 AI 學習與精準測驗":
    ui_components.render_header(
        "📚 AI 學習與精準測驗 (RAG)", 
        "選擇您的教材，讓 AI 針對內容產出精準考題，測試您的理解程度。"
    )
    
    # Onboarding panel
    if st.session_state['role'] == 'student':
        stats = calculate_gamification(st.session_state['username'])
        if not stats['onboarding_completed']:
            st.info("👋 **歡迎來到 SNA 學習中樞！** 請完成以下新手任務來獲得您的第一枚徽章：\n\n" +
                    f"{'✅' if stats['tasks'][0] else '⬜'} 任務 1：完成一場 AI 測驗\n\n" +
                    f"{'✅' if stats['tasks'][1] else '⬜'} 任務 2：收藏一張重點字卡\n\n" +
                    f"{'✅' if stats['tasks'][2] else '⬜'} 任務 3：挑戰一次臨床模擬 RPG")
        elif 'onboarding_just_completed' not in st.session_state:
            st.session_state['onboarding_just_completed'] = True
            st.balloons()
            st.success("🎉 恭喜您完成所有新手任務，獲得【🔰 菜鳥報到】徽章與 200 XP！")
    ...
    if st.session_state['exam_state'] == 'setup':
        ...
        with col_setup_l:
            ...
            # Difficulty
            difficulty = st.selectbox("4. 出題難易度：", ["🌱 初階 (護生：名詞與基礎概念)", "🌟 中階 (SNA：臨床流程與機轉)", "🔥 高階 (主治挑戰：罕見併發症)"], index=1)
            
            # Question count
            q_count = st.slider("5. 產生題數：", 3, 10, 5)
...
# --- PAGE 4: 📊 個人學習成果 ---
    ...
    with st.expander("🏆 成就解鎖指南 (看看您可以獲得哪些徽章)"):
        st.markdown(
            """
            *   🔰 **菜鳥報到**：完成新手引導任務 (完成1次測驗、收藏1張字卡、挑戰1次RPG)。
            *   🎯 **刷題狂人**：參與超過 5 次 AI 測驗。
            *   📚 **知識收藏家**：在學習過程中收藏超過 5 張重點字卡。
            *   🚀 **突飛猛進**：最新一次測驗成績，超越過去平均成績 20 分以上！
            *   🎓 **經驗值 (EXP) 說明**：完成一次測驗 +50 XP，獲得一分 +1 XP，收藏一張字卡 +10 XP，完成一次臨床 RPG +100 XP！
            """
        )
...
# --- PAGE: ⚙️ AI 糾錯與微調中心 ---
    ...
    ui_components.render_header(
        "⚙️ AI 糾錯與微調中心", 
        "在這裡新增最高指導原則，AI 出題與回答時會強制優先遵守此處的規則。"
    )
    ...
    with col1:
        ui_components.draw_card_start("➕ 新增糾錯規則")
        new_rule = st.text_area("在此輸入您要教導 AI 的規則：", placeholder="例如：提到 TCI 時，必須說明現代最新指引，不要使用 1990 年的舊文獻。")
        if st.button("💾 儲存新規則", use_container_width=True):
            if new_rule.strip():
                rules.append(new_rule.strip())
                database.save_ai_rules(rules)
                st.success("成功新增一條規則！")
                st.rerun()
        ui_components.draw_card_end()
        
    with col2:
        ui_components.draw_card_start("📋 目前生效的指導原則庫")
        if not rules:
            st.info("💡 目前尚無自訂規則。AI 將依據標準設定運作。")
        else:
            for i, r in enumerate(rules):
                st.markdown(f"**{i+1}.** {r}")
                if st.button("🗑️ 刪除", key=f"del_rule_{i}"):
                    rules.pop(i)
                    database.save_ai_rules(rules)
                    st.success("已刪除規則！")
                    st.rerun()
