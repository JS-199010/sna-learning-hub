...
def get_supabase_client() -> Client:
    ...
    if not url or not key:
        import streamlit as st
        st.error("⚠️ 系統找不到 Supabase 的連線金鑰！請確定您有在 Streamlit Cloud 後台的 Secrets 貼上設定，並且有按下 Save。")
        st.stop()
        
    print(f"DEBUG: Supabase URL length: {len(url)}, URL prefix: {url[:8] if url else 'None'}")
...
def search_chunks_vector(query_embedding, match_count=50, filename=None):
    """Call Supabase pgvector RPC to get top matching chunks."""
    params = {
    ...
    if filename and filename != "All Files" and filename != "All Uploaded Textbooks":
        ...
        params["filter_filename"] = None
        
    try:
        res = supabase.rpc("match_chunks", params).execute()
        # RPC should return table (filename text, chunk_text text, similarity float)
        return [(r['filename'], r['chunk_text'], r['similarity']) for r in res.data]
    except Exception as e:
        print(f"pgvector RPC Error: {e}")
        return []
