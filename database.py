...
def clear_score(score_id):
    """Sets a score value to NULL to 'delete' the score while retaining the exam record entry."""
    conn = get_connection()
    c = conn.cursor()
    c.execute("UPDATE scores SET score = NULL WHERE id = ?", (score_id,))
    conn.commit()
    conn.close()
...
def update_user_password(username, new_password):
    """Reset a user's password securely with SHA-256 hashing."""
    conn = get_connection()
    c = conn.cursor()
...
def update_user_role(username, new_role):
    """Promote or demote a user's account role."""
    conn = get_connection()
    c = conn.cursor()
